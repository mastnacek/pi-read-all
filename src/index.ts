...
import { formatBytes, formatScanResult, scanPath } from "./scanner.js";

/** Regex to match `@!"quoted path"` or `@!unquoted_path` */
const AT_EXCL_RE = /@!"([^"\n]+)"|@!([^\s"(){}[\];,]+)/g;

const COMMAND_DOCS = {
	"<path>": "Load file or directory recursively into context",
...
export default function (pi: ExtensionAPI): void {
	...
	// 2. Input interceptor: Transform @! triggers into full file/dir contents
	pi.on("input", async (event, ctx: ExtensionContext) => {
		...
		for (const match of matches) {
			...
			try {
				...
				const formattedContent = formatScanResult(scanResult, targetPath);

				newText = newText.replace(fullMatch, `\n\n${formattedContent}\n\n`);

				matchedCount++;
				loadedFilesCount += scanResult.files.length;
				loadedBytesCount += scanResult.totalBytes;
			} catch (error: unknown) {
				const msg = error instanceof Error ? error.message : String(error);
				ctx.ui.notify(`[pi-read-all] Could not read "${targetPath}": ${msg}`, "error");
			}
		}

		if (matchedCount > 0) {
			sessionStats.totalLoads += matchedCount;
			sessionStats.totalFilesLoaded += loadedFilesCount;
			sessionStats.totalBytesLoaded += loadedBytesCount;

			if (ctx.hasUI) {
				ctx.ui.notify(
					`[pi-read-all] Loaded ${loadedFilesCount} file(s) into context (${formatBytes(loadedBytesCount)})`,
					"info",
				);
			}
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			path: Type.String({
				description: "Relative or absolute path to the file or directory to read completely",
			}),
			maxFiles: Type.Optional(
				Type.Number({
					description: "Maximum number of files to read when scanning a directory (default: 2000)",
				}),
			),
			maxTotalBytes: Type.Optional(
				Type.Number({
					description: "Maximum total bytes to read across all files (default: 50MB)",
				}),
			),
		}),
		execute: async (_toolCallId, params, _signal, _onUpdate, ctx) => {
			try {
				...
				const msg = error instanceof Error ? error.message : String(error);
				return {
					content: [{ type: "text", text: `[pi-read-all] Error reading "${params.path}": ${msg}` }],
					details: { error: true },
				};
			}
		},
	...
	pi.registerCommand("read-all", {
		...
		getArgumentCompletions: async (prefix: string): Promise<AutocompleteItem[] | null> => {
			...
			if (tokens.length <= 1 && !/\s$/.test(prefix)) {
				const subcommands = Object.entries(COMMAND_DOCS).flatMap(([key, description]) =>
					key.toLowerCase().startsWith(typed) ? [{ value: key, label: key, description }] : [],
				);
		...
		handler: async (args: string, ctx: ExtensionCommandContext) => {
			...
			if (!subcommand || subcommand === "help" || subcommand === "-h" || subcommand === "--help") {
				const helpText = [
					...
					"Recursively load complete files or directories into LLM context with zero truncation.",
					"",
					"### Usage & Syntax:",
					...
					"",
					"### Slash Commands:",
					"  /read-all <path>     — Load path and inject content into next message",
					"  /read-all status     — Show session load metrics",
			...
			try {
				...
				ctx.ui.notify(
					`[pi-read-all] Loaded ${result.files.length} file(s) (${formatBytes(result.totalBytes)}). Injecting into conversation...`,
					"info",
				);
				...
				const msg = error instanceof Error ? error.message : String(error);
				ctx.ui.notify(`[pi-read-all] Failed to load "${targetPath}": ${msg}`, "error");
			}
		},
	});
}
